...
OSXWindow::OSXWindow(NSView * existingView) : mView(existingView), mDelegate(nil), mWindow(nil) {}
...
bool OSXWindow::initialize(const std::string &name, int width, int height)
{
   ...
   // mView.layer.contentsScale = 1;
    ...
    mMetalLayer.frame = CGRectMake(0,0,mView.layer.frame.size.width,mView.layer.frame.size.height);
               
    // Autoresize with parent layer.
    mMetalLayer.autoresizingMask = kCALayerWidthSizable | kCALayerHeightSizable;
    mMetalLayer.contentsScale = mView.layer.contentsScale;
    [mView.layer addSublayer: mMetalLayer];
...
void OSXWindow::messageLoopOnce()
{
    @autoreleasepool
    {
        NSEvent *event;
...
bool OSXWindow::setPosition(int x, int y)
{
    ...
    if(mWindow) {
        ...
        [mView setFrameOrigin:CGPointMake(x, YCoordToFromCG(newBottom))];
    }
