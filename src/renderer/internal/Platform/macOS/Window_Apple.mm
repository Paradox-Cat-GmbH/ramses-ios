...
Window_Apple::Window_Apple(const DisplayConfigData& displayConfig, IWindowEventHandler &windowEventHandler, UInt32 id)
        : Window_Base(displayConfig, windowEventHandler, id),
            mExternallyOwnedWindow(displayConfig.getMacOSNativeWindow())
    {
    ...
    bool Window_Apple::init()
    {
        ...
        runOnUIThread(^(void) {
            mWindow->initialize(m_windowName.c_str(), m_width, m_height);
